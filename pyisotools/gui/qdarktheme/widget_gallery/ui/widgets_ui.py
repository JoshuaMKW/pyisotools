...
from pyisotools.gui.qdarktheme.qtpy.QtCore import QAbstractTableModel, QModelIndex, Qt
from pyisotools.gui.qdarktheme.qtpy.QtGui import QIcon, QTextOption
...
class _Group1(QGroupBox):
    def __init__(self) -> None:
        ...
        push_btn, push_btn_toggled = QPushButton(
            "NORMAL"), QPushButton("TOGGLED")
        push_btn_flat, push_btn_flat_toggled = QPushButton(
            "NORMAL"), QPushButton("TOGGLED")
        tool_btn, tool_btn_toggled, tool_btn_text = QToolButton(), QToolButton(), QToolButton()
        radio_btn_1, radio_btn_2 = QRadioButton(
            "Normal 1"), QRadioButton("Normal 2")
        checkbox, checkbox_tristate = QCheckBox(
            "Normal"), QCheckBox("Tristate")
        ...
        tool_btn_text.setToolButtonStyle(
        ...
        for i in range(5):
            ...
            for j in range(2):
                item.addChild(QTreeWidgetItem(
                    [f"Child Item {i+1}_{j+1}" for _ in range(2)]))
...
class WidgetsUI:
    ...
    def setup_ui(self, win: QWidget) -> None:
        """Set up ui."""
        # Widgets
        h_splitter_1, h_splitter_2 = QSplitter(
            Qt.Orientation.Horizontal), QSplitter(Qt.Orientation.Horizontal)
