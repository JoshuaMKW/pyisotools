...
from pyisotools.gui.qdarktheme.qtpy import QtImportError, __version__
from pyisotools.gui.qdarktheme.qtpy.qt_compat import QT_API
from pyisotools.gui.qdarktheme.util import OPERATORS, compare_v, get_logger, get_qdarktheme_root_path, multi_replace
...
if __version__ is None:
    ...
    # Fairly future version for always setting latest version.
    _qt_version = "10.0.0"
else:
...
if QT_API is None:
    _qt_api = "PySide6"
    _logger.warning(
        f"Failed to detect Qt binding. Load Qt API as '{_qt_api}'.")
else:
    _qt_api = QT_API
...
def get_themes() -> tuple[str, ...]:
    ...
    Returns:
        Available themes.
    """
    from pyisotools.gui.qdarktheme.themes import THEMES
...
def load_stylesheet(theme: str = "dark") -> str:
    ...
    if theme not in get_themes():
        raise TypeError(
            "The argument [theme] can only be specified as 'dark' or 'light'.") from None
    ...
    try:
        ...
        if theme == "dark":
            from pyisotools.gui.qdarktheme.themes.dark import rc_icons as _
        else:
            from pyisotools.gui.qdarktheme.themes.light import rc_icons as _  # noqa: F401
        ...
        if hasattr(sys, "_MEIPASS"):
            icon_path = (Path(sys._MEIPASS) /
                         "qdarktheme").as_posix()  # type: ignore
        else:
            icon_path = get_qdarktheme_root_path().as_posix()

    if theme == "dark":
        from pyisotools.gui.qdarktheme.themes.dark.stylesheet import STYLE_SHEET
    else:
        from pyisotools.gui.qdarktheme.themes.light.stylesheet import STYLE_SHEET
...
def load_palette(theme: str = "dark"):
    ...
    if theme == "dark":
        from pyisotools.gui.qdarktheme.themes.dark.palette import PALETTE
    else:
        from pyisotools.gui.qdarktheme.themes.light.palette import PALETTE
    return PALETTE
